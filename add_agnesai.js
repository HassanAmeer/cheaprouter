...
compTs = compTs.replace(
  /(const anyrouterInst = checkProv\('ap_anyrouter'[\s\S]*?if \(anyrouterInst\) return anyrouterInst;\n)/,
  '$1' + check
);
fs.writeFileSync('backend/src/completions.ts', compTs);

// 5. Create Next.js proxy routes for agnesai
...
console.log('AgnesAI added successfully!');
