...
compTs = compTs.replace(
  /(const aihordeInst = checkProv\('ap_aihorde'[\s\S]*?if \(aihordeInst\) return aihordeInst;\n)/,
  '$1' + check
);
fs.writeFileSync('backend/src/completions.ts', compTs);

// 5. Create Next.js proxy routes for pollinations
...
console.log('Pollinations added successfully!');
