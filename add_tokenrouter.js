...
compTs = compTs.replace(
  /(const agnesaiInst = checkProv\('ap_agnesai'[\s\S]*?if \(agnesaiInst\) return agnesaiInst;\n)/,
  '$1' + check
);
fs.writeFileSync('backend/src/completions.ts', compTs);

// 5. Create Next.js proxy routes for tokenrouter
...
console.log('TokenRouter added successfully!');
