...
const prov = { 
  name: 'AnyRouter', 
  lower: 'anyrouter', 
  url: 'https://api.anyrouter.dev/v1', 
...
compTs = compTs.replace(
  /(const pollinationsInst = checkProv\('ap_pollinations'[\s\S]*?if \(pollinationsInst\) return pollinationsInst;\n)/,
  '$1' + check
);
fs.writeFileSync('backend/src/completions.ts', compTs);

// 5. Create Next.js proxy routes for anyrouter
...
console.log('AnyRouter added successfully!');
