...
export interface SambaNovaSetupRef {
...
const SambaNovaSetup = forwardRef<SambaNovaSetupRef, { onModelsUpdated?: () => void, index?: number }>(({ onModelsUpdated, index }, ref) => {
  ...
  const handleTestApi = async (index: number, silent = false) => {
    ...
    try {
      const res = await fetch(`/api/admin/sambanova/models?key=${encodeURIComponent(keyObj.key)}`, {
  ...
  useEffect(() => {
    fetch('/api/admin/sambanova', { headers: getAuthHeaders() })
      .then(res => res.json())
      .then(data => {
        if (data.key) {
          try {
  ...
  const handleFetchModels = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/sambanova/models?key=${encodeURIComponent(validKey)}`, {
        headers: getAuthHeaders()
      });
      const data = await res.json();
      ...
      console.error(e);
      alert('Failed to fetch SambaNova models.');
    } finally {
          ...
          {apiKeys.map((keyObj, index) => (
            <div key={index} style={{ display: 'flex', flexDirection: 'column', gap: '4px', opacity: keyObj.active ? 1 : 0.6 }}>
              <label style={{ fontSize: '13px', color: 'var(--color-text-muted)', fontWeight: 600, display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%' }}>
                ...
                {index === 0 && (
                  <a href="https://cloud.sambanova.ai/" target="_blank" rel="noopener noreferrer" style={{ display: 'inline-block', fontSize: '11px', color: 'var(--color-primary)', textDecoration: 'none', background: 'var(--color-bg-soft)', padding: '4px 8px', borderRadius: '6px', border: '1px solid var(--color-border)', fontWeight: 500 }}>
                    Get API Key ↗
                  </a>
                )}
              </label>
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', justifyContent: 'flex-end' }} onClick={handleDrawerClose}>
          <div style={{ width: '460px', background: 'var(--color-card-bg)', height: '100%', display: 'flex', flexDirection: 'column', boxShadow: '-5px 0 15px rgba(0,0,0,0.1)' }} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ padding: '10px 16px', borderBottom: '1px solid var(--color-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'var(--color-bg-soft)' }}>
              <h2 style={{ fontSize: '15px', fontWeight: 600, margin: 0 }}>SambaNova Models</h2>
