...
export interface HyperbolicSetupRef {
  testApi: (silent?: boolean) => Promise<boolean>;
}

const HyperbolicSetup = forwardRef<HyperbolicSetupRef, { onModelsUpdated?: () => void, index?: number }>(({ onModelsUpdated, index }, ref) => {
  ...
  const handleTestApi = async (index: number, silent = false) => {
    ...
    try {
      const res = await fetch(`/api/admin/hyperbolic/models?key=${encodeURIComponent(keyObj.key)}`, {
  ...
  useEffect(() => {
    fetch('/api/admin/hyperbolic', { headers: getAuthHeaders() })
      .then(res => res.json())
      .then(data => {
        if (data.key) {
          try {
  ...
  const handleFetchModels = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/hyperbolic/models?key=${encodeURIComponent(validKey)}`, {
        headers: getAuthHeaders()
      });
      const data = await res.json();
      ...
      console.error(e);
      alert('Failed to fetch Hyperbolic models.');
    } finally {
  ...
  const handleSave = async (modelsToSave = selectedModels, keysToSave = apiKeys, shouldNotify = false, overrideStatus: boolean | null = null, showUiFeedback = shouldNotify) => {
    ...
    try {
      ...
      const res = await fetch('/api/admin/hyperbolic', {
        method: 'PUT',
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '8px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <div style={{ width: '32px', height: '32px', borderRadius: '8px', background: 'var(--color-bg-soft)', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
              <img src="https://www.google.com/s2/favicons?domain=hyperbolic.xyz&sz=128" alt="Hyperbolic" style={{ width: '24px', height: '24px', objectFit: 'contain' }} />
            </div>
            <span style={{ fontSize: '18px', fontWeight: 600 }}>Hyperbolic</span>
          </div>
                    <div 
            onClick={() => {
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', justifyContent: 'flex-end' }} onClick={handleDrawerClose}>
          <div style={{ width: '460px', background: 'var(--color-card-bg)', height: '100%', display: 'flex', flexDirection: 'column', boxShadow: '-5px 0 15px rgba(0,0,0,0.1)' }} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ padding: '10px 16px', borderBottom: '1px solid var(--color-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'var(--color-bg-soft)' }}>
              <h2 style={{ fontSize: '15px', fontWeight: 600, margin: 0 }}>Hyperbolic Models</h2>
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', borderBottom: '1px solid var(--color-border)' }}>
              <div style={{ padding: '8px 16px', display: 'flex', gap: '8px', alignItems: 'center' }}>
                ...
                <a 
                  href={(() => { const k = apiKeys.find(k => k.active && k.key.trim() !== '')?.key || ''; return k ? "https://api.hyperbolic.ai/v1/models?key=" + encodeURIComponent(k) : "https://api.hyperbolic.ai/v1/models"; })()} 
                  target="_blank" 
