...
app.get('/api/admin/${prov.lower}/models', async (c) => {
  try {
    ...
    const res = await fetch('${prov.url}/status/models', {
...
compTs = compTs.replace(
  /(const modelscopeInst = checkProv\('ap_modelscope'[\s\S]*?if \(modelscopeInst\) return modelscopeInst;\n)/,
  '$1' + check
);
fs.writeFileSync('backend/src/completions.ts', compTs);

// 5. Create Next.js proxy routes for aihorde
...
console.log('AIHorde added successfully!');
